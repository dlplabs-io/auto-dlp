...
async function processProfile(profileId: string) {
  ...
  try {
    ...
    if (profileError || !profile?.dimo_dataregistry_relay_url) {
      throw new Error(`Profile not found or missing relay URL: ${profileError?.message}`);
    }
    ...
    while (attempts < MAX_POLLING_ATTEMPTS) {
      ...
      console.log(`Profile ${profile.id} task state is ${task.taskState}, polling... (attempt ${attempts + 1}/${MAX_POLLING_ATTEMPTS})`);
    ...
    for (const args of logs) {
      const fileData = {
        ...
        relay_url: profile.dimo_dataregistry_relay_url,
        proofTxn: null,
        proof: null
      };
...
export default async function handler(
  ...
  try {
    ...
    if (!profileIds || !Array.isArray(profileIds) || profileIds.length === 0) {
      return res.status(400).json({ error: 'Array of profile IDs is required' });
    }
